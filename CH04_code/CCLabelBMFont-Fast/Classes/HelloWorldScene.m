...
-(id) init
{
	if ((self = [super init]))
	{
		...
		if (useBrokenFont == NO)
		{
			scoreLabel = [CCLabelBMFont labelWithString:@"10" fntFile:@"arial40.fnt"];
		}
		else
		{
			...
			scoreLabel = [CCLabelBMFont labelWithString:@"10" fntFile:@"flipped-vertically.fnt"];
		}
...
-(void) updateInfoString
{
	...
	{
		case 0:
			[info setString:@"BMFont, not updating"];
			scoreLabel.color = ccGREEN;
			break;
		case 1:
			[info setString:@"BMFont, updating once per second"];
			scoreLabel.color = ccYELLOW;
			break;
		case 2:
			[info setString:@"BMFont, updating every frame"];
			scoreLabel.color = ccORANGE;
			break;
		case 3:
			[info setString:@"BMFont, updating 50 times per frame"];
